...
iOS 16.2 / arm64 / Dopamine rootless tweak. **Latest: 1.3.8 (session-aware rebind; GitHub macOS formal DEB built by run 34830950020)**
...
## 1.3.8 session-aware rebind（2026-09-14）

- `CGMBootstrap.js` 不再永久缓存首个账号的 `Manager/PlayerData/MapData/Pay/VipCard`：heartbeat 会重新读取当前对象，身份/引用变化时递增 `sessionGen`，清理旧 `cmd/res/state/probe`，恢复旧 Pay/iOS wrapper 并给新对象重新安装。
...
- 真实 App Store / StoreKit 交易回调、SKU 和线上购买路径保持不变。

验证：当前 mock 回归覆盖目标额度、重复点击幂等和末档加赠资格。
...
Expected current result: `22/22 tests passed`.
